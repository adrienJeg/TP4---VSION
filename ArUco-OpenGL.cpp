...
void ArUco::drawScene() {
   ...
   time(&timeCurrent);
	if ((choice && (m_Markers.size() == 2)) || (timeCurrent - timeInit > 1.5))
	{
		rand_id = rand() % 2;
		choice = false;
		time(&timeInit);
		//std::cout << "rand_id : " << rand_id << endl;
	}
	...
	{
		//std::cout << "id marqueur : " << m_Markers[m].id << endl;
		if (m_Markers[m].id == rand_id) // inutile ici car nous n'avons que deux tags, mais, nécessaire si l'on souhaite généraliser
		{
			rightTagVisible = true;
			...
			glPushMatrix();
			glutSolidSphere(m_MarkerSize/4, 10, 10);
   ...
   std::cout << "Points joueur : " << points_joueur << endl;
